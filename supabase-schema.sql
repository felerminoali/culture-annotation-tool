...
CREATE TABLE public.task_submissions (
    ...
    cultural_score INTEGER CHECK (
        cultural_score >= 0
        AND cultural_score <= 100
    ),
    language_similarity TEXT CHECK (
        language_similarity IN ('yes', 'no', 'na')
    ),
...
CREATE TABLE public.annotations (
    ...
    is_relevant TEXT CHECK (
        is_relevant IN ('yes', 'no', 'na')
    ),
    relevant_justification TEXT,
    is_supported TEXT CHECK (
        is_supported IN ('yes', 'no', 'na')
    ),
    supported_justification TEXT,
    culture_proxy TEXT,
    annotation_type TEXT CHECK (
        annotation_type IN ('manual', 'ai')
    ),
    subtype TEXT CHECK (
        subtype IN ('culture', 'issue')
    ),
    issue_category TEXT,
...
CREATE TABLE public.image_annotations (
    ...
    shape_type TEXT CHECK (
        shape_type IN ('rect', 'circle')
    ),
    description TEXT,
    comment TEXT,
    is_present TEXT CHECK (
        is_present IN ('yes', 'no', 'na')
    ),
...
-- Users policies
CREATE POLICY "Users can view all users" ON public.users FOR
SELECT USING (true);
...
-- Projects policies
CREATE POLICY "Users can view all projects" ON public.projects FOR
SELECT USING (true);
...
CREATE POLICY "Admins can update projects" ON public.projects FOR
UPDATE USING (
    EXISTS (
        SELECT 1
        FROM public.users
...
-- Tasks policies
CREATE POLICY "Users can view all tasks" ON public.tasks FOR
SELECT USING (true);
...
CREATE POLICY "Users can view own submissions" ON public.task_submissions FOR
SELECT USING (user_id = auth.uid ());

CREATE POLICY "Admins can view all submissions" ON public.task_submissions FOR
SELECT USING (
        EXISTS (
            SELECT 1
            FROM public.users
...
CREATE POLICY "Users can insert own submissions" ON public.task_submissions FOR
INSERT
WITH
    CHECK (user_id = auth.uid ());

CREATE POLICY "Users can update own submissions" ON public.task_submissions FOR
UPDATE USING (user_id = auth.uid ());

-- Annotations policies
CREATE POLICY "Users can view own annotations" ON public.annotations FOR
SELECT USING (user_id = auth.uid ());

CREATE POLICY "Admins can view all annotations" ON public.annotations FOR
SELECT USING (
        EXISTS (
            SELECT 1
            FROM public.users
...
CREATE POLICY "Users can view own image annotations" ON public.image_annotations FOR
SELECT USING (user_id = auth.uid ());

CREATE POLICY "Admins can view all image annotations" ON public.image_annotations FOR
SELECT USING (
        EXISTS (
            SELECT 1
            FROM public.users
...
CREATE POLICY "Users can view project assignments" ON public.project_assignments FOR
SELECT USING (true);
...
CREATE POLICY "Users can view task assignments" ON public.task_assignments FOR
SELECT USING (true);
